...
| `serve_my_kind_dns_`dnsmasq`_install`       | Boolean to install `dnsmasq` if not present              | `true`                                                      |
...
Assuming that the cluster name has been set to `cluster-local` and the resulting
domain is `cluster.local` for the services, the following Ingress resource
will be used to expose a service with the FQDN of `example.service.cluster.local`,
which will be accessible via the host machine's browser:
